...
class User(snscrape.base.Entity):
    ...
    verified_phone_status: typing.Optional[Boolean] = None
...
class Trend(snscrape.base.Item):
    ...
    def __str__(self):
        return self.name.replace(
            "#", ""
        )  # f'https://twitter.com/search?q={urllib.parse.quote(self.name)}'
...
class _TwitterAPIScraper(snscrape.base.Scraper):
    ...
    def _ensure_guest_token(self, url=None):
        while self._guestTokenManager.token is None:
            ...
            elif len(gts := re.findall("gt=\d+", r.text)) > 0:
                t = gts[-1]
                self._guestTokenManager.token = t[3:]
            else:
    ...
    def _get_api_data(self, endpoint, apiType, params, with_features=False):
        self._ensure_guest_token()
        cookies = None
    ...
    def _iter_api_data(
        ...
        while True:
            ...
            obj = self._get_api_data(endpoint, apiType, reqParams, with_features)
            yield obj
...
class TwitterUserScraper(TwitterSearchScraper):
    ...
    def _get_user_object(self, user):
        ...
        link = None
        if user.get("legacy", {}).get("url"):
            entity = (
                user["legacy"].get("entities", {}).get("url", {}).get("urls", [None])[0]
            )
            if not entity or entity.get("url") != user["legacy"]["url"]:
        ...
        label = None
        if labelO := user.get("affiliates_highlighted_label", {}).get("label"):
        ...
        return User(
            ...
            listedCount=user["legacy"].get("listed_count"),
            ...
            label=label,
            verified_phone_status=user.get("verified_phone_status"),
        )
    ...
    def _get_entity(self):
        ...
        obj = self._get_api_data(endpoint, _TwitterAPIType.GRAPHQL, params)
        if (
            not obj["data"]
        ...
        return self._get_user_object(user)
...
class TwitterUserScrapper(TwitterUserScraper):
    def __init__(self, user, **kwargs):
    ...
    def get_items(self):
        if not self._isUserId:
            ...
            userId = self._user
        self._apiHeaders["Referer"] = f"https://twitter.com/{self._user}/followers"
        ...
        for obj in self._iter_api_data(
            ...
            try:
                instruction = obj["data"]["user"]["result"]["timeline"]["timeline"][
                    "instructions"
                ][-1]
            except KeyError:
                try:
            ...
            for ent in entries[:-2]:
                try:
                    user = ent["content"]["itemContent"]["user_results"]["result"]
                    user["legacy"]
                except KeyError:
                    continue
                yield self._get_user_object(user)
